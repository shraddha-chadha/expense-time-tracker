...
module.exports = async (req, res) => {
  ...
  console.log("===> GET QUARTERLY EXPENSE", slots);
  ...
  const response = await fetch(totalURL, options).then(async (response) => {
    ...
    if(results.status === 404) {
      ...
      response: {
        outputSpeech: {
          type: "SSML",
          ssml: `<speak>Your quarterly expense for quarter ${slots.quarter.value} ${slots.year.value} is $${Number(results.totalExpense).toFixed(2)} </speak>`
        }
      }
      })
          }
